...
- **I-P1 — Um caso de uso alcançável por HTTP e por MCP tem exatamente uma declaração de contrato de entrada.** Os campos que um chamador pode enviar, seus tipos e seus limites vivem uma vez só, em `src/<bc>/presentation/schema/<caso_de_uso>.schema.ts`, como um `z.ZodRawShape` — shape cru porque é o que `McpToolDefinition.inputSchema` recebe; o controller embrulha com `z.object(shape)`. Todo `.max()` numérico referencia a constante de `domain`, nunca um literal, e o `.describe()` mora no shape, servindo à IA e ao `/docs` pelo mesmo texto. Diferença legítima entre os canais é um `.extend()`/`.omit()` visível no consumidor, nunca uma segunda cópia do campo — as duas que existem hoje são a data (`z.coerce.date()` no HTTP contra ISO-8601 com offset obrigatório no MCP, porque uma IA inventa datetime sem fuso) e o `entrance_code` (só HTTP: é senha real de fechadura e nunca pode entrar no contexto de uma IA). Sem a trava as duas cópias divergem em silêncio, e já tinham: `capacity`/`guests` a 500 no HTTP contra 1000 no MCP, `tenant.name` com `min(2)` no HTTP contra o `min(3)` que a entidade exige — um nome de duas letras virava 500 em vez de 422 —, `tenant.phone` com formato BR fixo de um lado e o do domínio do outro, `list_tenants` sem validação nenhuma no HTTP, e `MAX_PROPERTY_IMAGES` redeclarado dentro de uma tool. Regra entre campos — a que restringe o objeto inteiro, não um campo — não cabe num shape cru, então é declarada ao lado dele como um `InputRule` e aplicada pelos dois consumidores (`withRules` no controller, `assertRules` no handler da tool, porque `inputSchema` de tool é shape cru e não tem onde pendurar refine); um `.refine()` escrito direto em `presentation/controller/` ou `presentation/mcp_tool/` é falha de teste. Sem isso o furo é silencioso e assimétrico: `list_stays` aceitava `from > to` e respondia lista vazia — uma IA concluía "não há estadias no período" — enquanto o HTTP devolvia 422. Travada pela regra ESLint `sogio/no-inline-input-schema` (que prova que existe uma declaração só) mais um teste de contrato entre canais (que prova que os dois aceitam os mesmos valores — o que a lint não consegue provar). Schema de saída fica fora: `outputSchema` existe só para o `/docs` e a tool não publica nenhum
