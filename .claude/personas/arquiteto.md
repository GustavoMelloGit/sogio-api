...
| ----------------------- | ---------------------------------------------------------------------------------------------------------------------- |
| **Auth**                | Identidade e autenticação de usuários, inclusive por identidades vinculadas a provedores externos (entrada com Google) |
...
- **LinkedIdentity** (Auth) — o par (provedor, identificador no provedor) ligado a uma conta; nada além disso, nem email nem nome. Único em (provedor, `subject`) e em (`user_id`, provedor): uma conta nunca acumula uma segunda identidade do mesmo provedor
- **ExternalSignInRequest** (Auth) — o estado transitório da entrada com um provedor externo; curto (10 minutos), de uso único, amarrado ao navegador que o iniciou por um cookie que nunca é persistido. Sem `user_id` e sem dado pessoal
...
- **I-N2 — A caixa de entrada só mostra o que foi entregue.** `GET /notifications` filtra `status === "sent"`; uma notificação `pending` ou `failed` nunca aparece na listagem, porque `markRead()` lança `IllegalStateError` (500) numa linha que não é `sent` — mostrar uma linha não-`sent` entregaria ao usuário um botão que quebra. `sent` é terminal (`markFailed` lança numa notificação já enviada), então "está na lista ⟹ dá para marcar como lida" vale para sempre, não só no instante da leitura
...
- **IE-1 — Nenhum corpo de requisição é materializado sem teto, e nenhum corpo recusado é abandonado pela metade.** Toda superfície de entrada (adaptador HTTP, `/mcp` e o leitor de CSV) limita bytes **enquanto lê**, sem consultar `Content-Length` — header opcional e escolhido pelo cliente nunca decide qual caminho o servidor toma. São dois números: quanto pode ser **retido** (`MAX_BUFFERED_BODY_BYTES`) e quanto pode ser **lido**, retido ou descartado (`MAX_REQUEST_BODY_BYTES`) — o primeiro é 1 MB e não 8 MB porque 1 MB de entrada não é 1 MB de memória: depois do parse a amplificação de heap já é de dezenas de MB, e é essa amplificação que justifica reter pouco mesmo permitindo ler mais. O `maxRequestBodySize` do `Bun.serve` **não é trava**: o Bun só o aplica quando há `Content-Length` e o ignora sob `chunked` (medido), então ele é atalho para o chamador honesto e nada mais — quem garante é a contagem no leitor. Cruzado o teto de retenção, o leitor para de reter e passa a **drenar e descartar** até o fim do stream, lançando só então: responder sem consumir o corpo dessincroniza a conexão keep-alive no Bun (`cancel()` não a recupera, `Connection: close` não é honrado, ambos medidos), e a requisição seguinte do mesmo cliente pendura até o timeout — um cliente legítimo que erra o tamanho uma vez fica com a conexão envenenada, falha mais frequente e mais desnorteante que a original. O dreno tem orçamento: estourado ele, para-se de ler e aquela conexão é dada como perdida — cortesia é para quem errou, não para quem fluxa sem parar. O pico de memória segue sendo um chunk. `bodyMode: "stream"` é a única saída para quem precisa aceitar mais, e ela troca "corpo maior" por "leitura incremental", nunca por "sem teto" nem por "sem dreno". Toda rota `bodyMode: "stream"` declara o próprio teto e drena no caminho de recusa — a garantia genérica do adaptador não a alcança, porque para ela não há corpo bufferizado a limitar. Nem lint nem guarda de boot consegue provar que um controller conta bytes: a guarda de boot só recusa `stream` junto com `inputSchema`, o que barra uma classe de erro (rota sem schema para validar um corpo que não existe), mas não prova que o controller lê com teto. Esta metade da invariante é obrigação humana, verificada em revisão — é o mais forte que se consegue sem inventar um mecanismo que não existe, e é melhor ser honesto sobre isso do que sugerir uma trava que não está lá
- **IE-2 — Nenhum texto que esta API converte em árvore JSON o faz sem teto de profundidade.** A varredura léxica roda antes do `JSON.parse`, nas duas superfícies. **O que ela não faz: impedir estouro de pilha, que não existe** — o `JSON.parse` do JavaScriptCore é iterativo, e 1 milhão de níveis parseiam em 27 ms sem erro (medido). **E não é ela que limita memória** — a amplificação de heap do parse (~64 bytes por container) depende de _quantos_ containers cabem nos bytes, não de como estão aninhados: 350 mil `[]` irmãos a 2 níveis custam a mesma ordem de heap que 500 mil aninhados; isso é assunto do teto de retenção de IE-1, não deste. O que este teto faz é limitar **recursão dependente de profundidade sobre dado vindo da requisição** — `JSON.stringify`, `structuredClone`, qualquer caminhada como `serializeDatesRecursively` — que sem ele estoura `RangeError` e vira `500` em vez de `422`, e fechar uma dimensão que de outro modo fica sem limite nenhum, por 1,8 ms por MB. Nenhum caminho de crash concreto foi demonstrado no código de hoje: esta é uma trava barata e preventiva, não crítica, e quem for recalibrar `MAX_JSON_DEPTH` precisa saber disso para não raciocinar contra uma ameaça que não existe.
- **IA-1 — Uma identidade vinculada é identificada por (provedor, `sub`), nunca por email.** `resolveExternalAccount` começa pelo `sub` e, quando ele já está vinculado, a decisão é final: o email que o provedor mandar naquele momento é ignorado. O email do provedor só é lido na hora de vincular ou de criar a conta, e nunca sincroniza com `User.email`
- **IA-2 — Email atestado é pré-condição de vínculo e de criação, e só nesses dois casos.** `email_verified` diferente do booleano literal `true` (ausente, `false`, string `"true"`) é tratado como não verificado. Fail-closed, no mesmo espírito de I-4: ausência nunca libera. `true` sozinho também não basta: Google só é autoridade sobre o email quando ele termina em `@gmail.com` (sem distinção de caixa no domínio) ou quando o claim `hd` chega como string não vazia (Google Workspace) — fora disso, a posse atual do endereço pode ter mudado de dono desde a última verificação, e a identidade atestada sai com `email_verified: false`. Um `hd` de tipo errado conta como ausente, nunca invalida o token inteiro
- **IA-3 — Conta sem senha nunca autentica por senha, e a recusa é indistinguível de senha errada.** `SignInUseCase` responde a mesma mensagem e o mesmo `401` para email inexistente, senha errada e conta sem senha
- **IA-4 — A primeira senha de uma conta sem senha só nasce pela recuperação por email, nunca pela troca autenticada.** `ChangePasswordUseCase` responde `409` para uma conta sem senha antes de qualquer comparação de hash: uma sessão roubada não pode plantar uma credencial que sobrevive à revogação de sessões
- **IA-5 — O pedido de entrada externa (`ExternalSignInRequest`) é de uso único e amarrado ao navegador.** É consumido atomicamente na primeira apresentação do `state`, qualquer que seja o desfecho, e só avança junto com o verificador PKCE que só o navegador que o iniciou possui (o cookie `google_sign_in_verifier`, nunca persistido). Nada do que o banco guarda completa o fluxo sozinho
- **IA-6 — As rotas de entrada com Google só redirecionam para dois lugares.** O endpoint de autorização do provedor (fixo em código) e a rota de resultado do front (`FRONT_BASE_URL` + caminho fixo, `GOOGLE_SIGN_IN_RESULT_PATH`). O destino de retorno (`return_to`) viaja como parâmetro dessa rota, nunca vira destino de redirect da própria API — sem isso a rota seria um redirect aberto
- **IA-7 — Um ID token só é aceito quando chega pelo canal direto com o endpoint de token do provedor.** É a premissa que dispensa verificar a assinatura em `parseGoogleIdTokenClaims` (OpenID Connect Core 1.0, §3.1.3.7, item 6). Qualquer fluxo futuro que receba um ID token pelo navegador (One Tap, app nativo) rompe essa premissa e exige verificação de assinatura com biblioteca, nunca escrita à mão. A premissa depende de TLS de verdade nos dois pontos que a sustentam: `NODE_TLS_REJECT_UNAUTHORIZED` nunca pode existir no processo da API, e `GoogleIdentityProvider.exchangeCode` nunca segue redirect (`redirect: "error"` no `fetch` do endpoint de token) — um 307/308 reenviaria `client_secret` e `code_verifier` a outro host, e um TLS não verificado abriria a troca de código inteira a um MITM
- **IA-8 — Nenhuma credencial do provedor é persistida nem logada.** Access token, refresh token e ID token do Google nunca tocam o banco nem o log; o único dado de perfil guardado além de `sub` e provedor é nome e email, e só no instante em que uma conta é criada pelo Google — um vínculo a uma conta existente não grava nada do perfil

### Vocabulário de Entrada Externa

- **Provedor de identidade** — sistema externo que atesta quem a pessoa é. Hoje só `google` (`IDENTITY_PROVIDERS`)
...
- **Pedido de entrada externa** (`ExternalSignInRequest`) — o estado transitório do fluxo; irmão de "Pedido de Recuperação de Senha" e de "Pending Authorization Request"
- **Vínculo automático** — ligar uma identidade a uma conta existente pelo email que o provedor atesta, sem tela de confirmação
- **Conta sem senha** — conta cuja única forma de entrada é uma identidade vinculada, até definir senha pela recuperação
- **Destino de retorno** (`return_to`) — caminho relativo do front para onde a pessoa volta depois do fluxo
- **Resultado da entrada** — o que o front recebe no fim (`status` ou `error`, na rota fixa de resultado)
- Evitar em código novo: "OAuth", "social login", "token do Google" — "OAuth" já significa o servidor de autorização do MCP, e "token" já significa credencial OAuth ou segredo de sessão
...
| `IdentityLinkedEvent`            | `CompleteExternalSignInUseCase`, quando uma identidade Google é vinculada a uma conta já existente (Auth) | Notificação `identity_linked` por email (Notification), avisando a dona da conta do vínculo — mitigação de R-1 |
