...
- **IA-2 — Email atestado é pré-condição de vínculo e de criação, e só nesses dois casos.** `email_verified` diferente do booleano literal `true` (ausente, `false`, string `"true"`) é tratado como não verificado. Fail-closed, no mesmo espírito de I-4: ausência nunca libera
