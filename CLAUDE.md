...
**Profundidade.** Todo corpo JSON passa por uma varredura léxica (`MAX_JSON_DEPTH = 32`) antes do `JSON.parse` — aninhamento profundo cabe em poucos KB, então o teto de bytes não cobre isso, e o estouro de pilha derruba o processo inteiro. Excedê-lo é `422`, não `413`: não é problema de tamanho.
