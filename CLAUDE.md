...
**Rate limiting** — dois campos porque são dois momentos que protegem coisas diferentes. `Controller.rateLimitPolicy` roda no `BunHttpControllerAdapter` **antes** de autenticação, chaveado por IP do peer (`peer-ip`) ou por uma chave que a própria rota já tem em mãos (`caller-key`, ex.: `/token`/`/revoke`) — existe para conter flood anônimo e nunca sabe quem é o usuário. `Controller.userRateLimitPolicy` roda **depois** que `AuthMiddleware` resolve o usuário e depois do portão de platform-access/capacidade, chaveado por `user.id` (dimensão `"user"`) — é o que de fato limita uma conta: dez pessoas atrás do mesmo IP deixam de dividir cota, e a mesma pessoa trocando de IP deixa de ganhar cota nova. Declarar `userRateLimitPolicy` numa rota que não é `authenticated` nem `adminOnly` derruba o boot, no mesmo espírito da checagem já existente para `requiredCapability` — a checagem nunca rodaria. `/mcp` ganhou o mesmo tratamento no seu próprio ponto de entrada, fora do `BunHttpControllerAdapter`: `makeMcpRequestHandler` aplica 300 requisições/minuto por usuário logo depois de `identityResolver.resolveRequester`, antes do portão de platform access — antes disso o transporte MCP inteiro não tinha limite nenhum. Logo depois do portão de platform access, `/mcp` também aplica a capacidade `ai_assistant` sobre o mesmo `Entitlement` já resolvido ali — nunca uma segunda ida ao banco —, dentro do mesmo bloco `role !== "admin"` que já pula as duas checagens para admin (ver "Capacidades de plano" abaixo). Abaixo do transporte, uma tool MCP pode declarar seu próprio `rateLimitPolicy` em `McpToolDefinition`, aplicado por `registerMcpTool` antes da checagem de capacidade, chaveado por `tool:<nome>:<user.id>`: `import_properties`/`import_stays`/`import_ledger_entries` a 30/hora (até `MAX_MCP_IMPORT_RECORDS = 100` registros por chamada) e `record_revenue`/`record_expense`/`book_stay` a 120/hora — a mesma trava fecha o caminho de uma IA contornar o paywall de `bulk_import` chamando `record_revenue` em loop. As rotas HTTP equivalentes ganharam `userRateLimitPolicy` com os mesmos números por escrita unitária (120/hora) e 10/hora para os três importadores (até `MAX_IMPORT_ROWS = 1000` linhas por requisição) — o teto por chamada é maior no HTTP, então o volume/hora resultante não precisa ser igual entre os dois transportes. Uma rejeição de tool nunca vira "Internal server error": `TooManyRequestsError` (`core/application/error/`) entrou no allow-list de `mcp_error_mapper.ts` e em `errorCodeMap` do adaptador HTTP como `429`, para os dois transportes concordarem.

**Um rate limit limita a taxa, não o total.** Nenhum dos mecanismos acima — `userRateLimitPolicy`, o limite de `/mcp`, o `rateLimitPolicy` de tool — impõe um teto de quanto uma conta pode escrever por dia, só quão rápido. É proteção contra abuso e contra uma IA em loop, não substituto de uma cota de volume: se o produto precisar limitar o total, é um mecanismo novo, não uma extensão deste.

**Superfície MCP obrigatória** — todo caso de uso ou endpoint novo **de escopo de usuário** nasce com a **tool MCP correspondente, na mesma entrega**. O produto tem que funcionar independente de UI: o backend concentra as regras de negócio e uma IA conectada ao `/mcp` deve conseguir executar todas as ações **do próprio usuário**. As tools ficam em `src/<bc>/presentation/mcp_tool/` e são registradas em dois pontos (factory `make<X>Tool()` no Di do BC, array `tools` de `makeMcpRequestHandler`), sempre sobre as **mesmas instâncias de DI** que o HTTP usa. Capacidade de plano decide **quem alcança** essa superfície, nunca **se ela nasce**: a capacidade `ai_assistant` (ver "Capacidades de plano") fecha o transporte `/mcp` inteiro para o Free, mas uma tool nova continua obrigatória na mesma entrega mesmo que hoje só o Pro chegue até ela.
...
**`ai_assistant` governa quem conecta uma IA à própria conta — em dois pontos.** Fecha o transporte `/mcp` inteiro (`makeMcpRequestHandler`) e o consentimento OAuth que emite o token dele (`DecideAuthorizationRequestUseCase`), nunca "tool a tool". É `kind: "access"`, `default: false`, `required: false`: fail-closed como `bulk_import` — um Price cujo `metadata` não traga `sogio_ai_assistant` deixa o plano sem IA em vez de invalidar a entrada de catálogo inteira, e a ausência nunca derruba o plano Pro por conta de uma chave esquecida. Chama-se `ai_assistant`, não `mcp_access`: o negócio vende "usar o Sogio por uma IA", não o nome de um protocolo — o `sogio-wa-bot` (projeto irmão, Baileys + Gemini) já consome esta API por MCP, e um segundo transporte para a mesma alavanca comercial não pede uma segunda capacidade. Não é uma exceção nova às exceções documentadas do MCP (material de credencial, o protocolo OAuth, webhooks, etc.) — é uma trava comercial sobre a superfície, no mesmo espírito de `bulk_import`. **A trava cobre o transporte MCP, não a API HTTP.** Uma IA que tenha o JWT de sessão do próprio usuário continua podendo dirigir a conta por HTTP. É limite comercial deliberado do desenho, não furo.

A matriz free/trial/pago sai de **um único fato do gateway** (`sogio_ai_assistant` no `metadata` de cada Price), sem nenhum ramo `if (status === "trialing")` em lugar nenhum do código: uma `Subscription` em `trialing` já resolve `CapabilitySet.of(plan.capabilities)` do **plano pago sendo experimentado**, então o trial herda a capacidade de graça. Quem corta o MCP em cada percurso: trial vencido e `past_due` fora da carência caem no portão de platform access de sempre, com o `blocked_reason` inalterado; uma assinatura **cancelada com o período pago já encerrado** continua com `has_platform_access: true` mas cai para as capacidades do plano **Free** — é o único caso em que só a trava de capacidade barra, porque o portão de platform access deixaria passar. **Downgrade nunca revoga token nem Consentimento**: o corte é sempre na leitura do `Entitlement`, a cada requisição — um access token emitido no plano pago continua sendo uma credencial válida depois do downgrade e a chamada seguinte já toma `403`; um upgrade de volta restaura o acesso na requisição seguinte, sem reconectar a IA. **`/token` com `grant_type=refresh_token` é deliberadamente cego a plano.** Uma conta que foi Pro, consentiu e caiu para free continua rotacionando refresh tokens e recebendo access tokens novos enquanto o `Consent` for utilizável. Isso é inofensivo hoje porque toda chamada ao `/mcp` toma `403`, mas significa que a superfície de credencial viva de uma conta sem direito nunca encolhe. A única aplicação da trava é a leitura de entitlement no transporte.

**Uma capacidade nova só existe no catálogo depois que o registro que a conhece está em produção _e_ o catálogo foi ressincronizado.** `parseCapabilities`/`SyncPlanCatalogEntryUseCase` iteram o `CAPABILITY_REGISTRY` do **binário em execução**; nenhuma sincronização feita pelo binário antigo grava uma chave nova, por mais correto que esteja o dashboard, e até a ressincronização toda conta cai no `default` do registro para aquela capacidade. O gatilho confiável é `POST /billing/catalog/sync` (`ReconcilePlanCatalogFromGatewayUseCase`, que ignora staleness e relê o catálogo inteiro), nunca o webhook — que só dispara quando o Price é editado depois do deploy. Isso já valia, sem estar escrito, para `bulk_import` e `export_reports`; ficou visível com `ai_assistant` porque ali o `default: false` **tira** acesso de contas pagas em vez de manter desligado um recurso que ninguém tinha ainda.
...
**Teto de corpo de requisição.** São dois números com significados diferentes: `MAX_BUFFERED_BODY_BYTES` (1 MB) é quanto se **retém** de um corpo não-stream — aplicado no `ControllerRequestParser` e no boundary do `/mcp` —, e `MAX_REQUEST_BODY_BYTES` (8 MB) é quanto se **lê** de qualquer corpo, retido ou descartado. O segundo precisa ficar estritamente acima de `MAX_IMPORT_BYTES` (5 MB), senão um CSV pouco maior que o teto seria cortado antes de receber o relatório `422` da importação. As três rotas `bodyMode: "stream"` não passam pelo teto de retenção — seguem com os próprios (`MAX_IMPORT_BYTES`, `MAX_IMPORT_FIELD_BYTES`, `MAX_IMPORT_ROWS`).

**Nenhum dos dois é aplicado pelo runtime, e isso é deliberado.** `Content-Type`, `Content-Length` e o `maxRequestBodySize` do `Bun.serve` são todos escolhidos pelo cliente ou dependentes de escolha dele: o Bun **só aplica `maxRequestBodySize` quando há `Content-Length`, e o ignora por completo sob `Transfer-Encoding: chunked`** (medido: 2 MB atravessaram um teto de 256 KB). Ele continua configurado porque corta o caso honesto antes de acordar o handler, mas nunca é a garantia — quem garante é a contagem byte a byte no leitor. Quem reencontrar essa limitação do Bun não deve "consertar" o teto: ele já está no lugar certo.

**Corpo recusado é lido até o fim, não abandonado.** Cruzado o teto de retenção, o leitor para de reter e continua lendo, descartando chunk a chunk, e só então lança `PayloadTooLargeError` (`413`). Não é zelo: responder sem consumir o corpo dessincroniza a conexão keep-alive no Bun — `cancel()` não a recupera e `Connection: close` não é honrado —, e a requisição **seguinte** do mesmo cliente pendura até o timeout. Trocar o DoS por "quem erra o tamanho uma vez fica com a conexão envenenada" seria negócio pior: atinge quem não está atacando ninguém e o sintoma aparece longe da causa. O dreno tem orçamento (`MAX_REQUEST_BODY_BYTES`); estourado, para-se de ler e a conexão é dada como perdida. Um `AbortError` no meio do dreno vira `413` se o teto já havia sido cruzado, e sobe como está se não havia — nesse caso é cliente que desistiu, e fabricar um `413` mentiria no log. O mesmo dreno vale no caminho de rejeição de `readCsvRecordStream`.

**Profundidade.** Todo corpo que esta API converte em árvore passa por uma varredura léxica (`MAX_JSON_DEPTH = 32`) antes do `JSON.parse`. Não é defesa contra estouro de pilha — o `JSON.parse` do JavaScriptCore é iterativo e parseia 1 milhão de níveis em 27 ms — nem contra memória: a amplificação de heap do parse (~64 bytes por container) segue a contagem de containers, não o aninhamento, e quem a limita é o teto de retenção de 1 MB, não este. O que a guarda limita é recursão dependente de profundidade sobre dado da requisição (`JSON.stringify`, `structuredClone`, `serializeDatesRecursively`), que sem ela estoura `RangeError` e vira `500` em vez de `422`. Excedê-la é `422`, não `413`: não é problema de tamanho. Cobre o que **esta API** parseia — um consumidor de `rawBody` que faça o próprio parse (hoje só o webhook do Stripe, que verifica assinatura antes de parsear) não passa por ela, e todo consumidor novo de `rawBody` precisa passá-la explicitamente.
...
rota só quebraria para quem usa o app.
