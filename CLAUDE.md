...
**Não há BC `import`.** "Importação" não tem agregado — um lote nasce e morre dentro de uma requisição, não tem id, não é persistido — e um BC sem agregado seria um pacote de código com nome de contexto. O mecanismo genérico mora em `core/application/import/` (`SourceRecord`, `ImportFailure`, `ImportReport`, `ImportRejectedError`, `ImportRunner`) e em `core/infra/http/csv/` (o parser incremental); cada BC dono tem o próprio caso de uso importador (`ImportPropertiesUseCase`, `ImportStaysUseCase`, `ImportLedgerEntriesUseCase`), que decide o que é um registro válido daquela entidade e reusa as próprias policies. Nenhuma dependência nova entre BCs nasce daqui: `core` já importava de `auth` e de `billing`, e `core/application/import/` não importa nada de nenhum BC — opera sobre um `AsyncIterable` de registros opacos e um callback.
