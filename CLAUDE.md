...
O primeiro evento ligado é `SubscriptionPaymentFailedEvent`, deliberadamente **não** os de estadia: `StayCanceledEvent` está sob a invariante DA-13/R-15 e travado por teste, e mexer nela merece PR própria. `notifications.user_id` e `notification_preferences.user_id` referenciam `users` com `ON DELETE cascade`, então o purge LGPD já leva tudo junto — há teste travando isso.
