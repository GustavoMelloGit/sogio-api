...
describe("POST /finance/:property_id/revenue", () => {
  beforeEach(async () => {
    await truncate(TABLES);
  });

  it("204 — records revenue for a property owned by the user", async () => {
  ...
  it("404 — rejects non-existent property_id", async () => {
    ...
    const res = await api(`/finance/${fakePropertyId}/revenue`, {
      method: "POST",
      headers: { Authorization: "Bearer " + token },
      body: JSON.stringify(validBody),
    });
